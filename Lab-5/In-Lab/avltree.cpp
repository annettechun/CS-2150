...
int AvlTree::int_path_length(AvlNode *t, int depth) {
  //depth: how far a node is in the tree -> root node has depth 0
  //frequency: number of nodes*depth
  //internal path length: sum of frequency column

  //sum of the depths of all nodes in a tree
    if (t == NULL){
      return 0;
    }
    ...
    return (depth + int_path_length(t -> left, depth+=1) + int_path_length(t -> right, depth));
}
